...
public class InoculationSearchHandler implements ActivityHandler<SearchRequest, SearchResponse<Inoculation>> {
    private final Logger LOG = LoggerFactory.getLogger(InoculationSearchHandler.class);
    ...
    public SearchResponse<Inoculation> handle(SearchRequest activity) {
        ...
        List<Inoculation> inoculations = cemaInoculations.stream().map(inoculationMapper::mapEntityToDomain).collect(Collectors.toList());
